...
namespace FunWithLocal.WebApi.Auth
{
    ...
    {
        private readonly ILogger<ArticleAuthorizationHandler> _logger;

        public ArticleAuthorizationHandler(ILogger<ArticleAuthorizationHandler> logger)
        {
            _logger = logger;
        }

        protected override Task HandleRequirementAsync(AuthorizationHandlerContext context, OperationAuthorizationRequirement requirement, Article article)
        {
            ...
            else
            {
                ...
                {
                    //Author
                    if (Convert.ToInt32(userId) == article.AuthorId)
                    {
                        context.Succeed(requirement);
                    }
                    else
